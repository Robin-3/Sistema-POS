import { z } from 'zod';
import { Clients, EnumGender, EnumIdentification, EnumRole, Sellers, Suppliers, Users } from './database.mjs';
...
export class Validations {
  ...
  static identificationId (identificationId) {
    ...
    const identification = EnumIdentification.findOne({ _id: identificationId });
  ...
  static identificationNumber (identificationNumber) {
    const numberSchema = z
      .string()
      .min(3, { message: 'The identification number must be more than 3 characters' });
   ...
   * }} options
   */
  static user ({ id, identificationId, identificationNumber, image, createdAt, updatedAt }) {
    Validations.id(id);
  ...
  static taxRegimeCode (taxRegimeCode) {
    const taxRegimeCodeSchema = z
      .string()
      .min(3, { message: 'The tax regime code must be at least 3 characters long' })
      .optional();
  ...
  static economicActivityCode (economicActivityCode) {
    const economicActivityCodeSchema = z
      .string()
      .min(3, { message: 'The economic activity code must be at least 3 characters long' })
      .optional();
   ...
   * @returns {string} id
   */
  static seller ({ id, names, surnames, genderId, roleId, password, taxRegimeCode, economicActivityCode }) {
    Validations.id(id);
